...
use crate::{common::{BitArray, GlobalHistogramBinarizer}, BinaryBitmap, BufferedImageLuminanceSource, oned::rss::expanded::RSSExpandedReader};
...
use super::bit_array_builder;
  ...
  fn assertCorrectImage2binary( fileName:&str,  expected:&str)
    ...
    let row = binaryMap.getBlackRow(rowNumber, &mut BitArray::new()).expect("row");

    // let pairs = Vec::new();
    // try {
      let mut rssExpandedReader =  RSSExpandedReader::new();
     let pairs = rssExpandedReader.decodeRow2pairs(rowNumber as u32, &row).expect("decode");
