...
pub fn checkStandardUPCEANChecksum(s: &str) -> Result<bool> {
    ...
    Ok(calculated_checksum
        == if check {
            char_in_question
                .to_digit(10)
                .ok_or(Error::checksum_with("checksum could not be verified"))?
        } else {
            u32::MAX
        })
}
...
pub fn getStandardUPCEANChecksum(s: &[char]) -> Result<u32> {
    ...
    while i >= 0 {
        ...
        let digit = (*s
            .get(i as usize)
            .ok_or(Error::internal_with("index out of bounds"))? as i32)
            - ('0' as i32);
        if !(0..=9).contains(&digit) {
            return Err(Error::format_with("digit is not between 0 and 9"));
        }
        sum += digit;
