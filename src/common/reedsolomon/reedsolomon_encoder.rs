...
impl ReedSolomonEncoder {
    ...
    pub fn encode(&mut self, to_encode: &mut [i32], ec_bytes: usize) -> Result<()> {
        if ec_bytes == 0 {
            return Err(Error::checksum_with("no error correction bytes"));
        }
        let data_bytes = to_encode.len() - ec_bytes;
        if data_bytes == 0 {
            return Err(Error::checksum_with("no data bytes provided"));
        }
        let fld = self.field;
        ...
        let remainder = &info.divide(generator.ok_or(Error::CHECKSUM)?)?.1;
