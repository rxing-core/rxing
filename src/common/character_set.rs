        ...
        match self {
            CharacterSet::Cp437 => {
                ...
                input
                    ...
                    .map_err(|e| Error::Format {
                        message: "cp437 encoding error".into(),
                        source: Some(
                            format!(
                                "cp437 error, data is representable up to: {}",
                                e.representable_up_to
                            )
                            .into(),
                        ),
                    })
            ...
            CharacterSet::ASCII => {
                ...
                for c in input.chars() {
                    if c as u32 > 0x7F {
                        return Err(Error::Format {
                            message: "ASCII encoding only supports characters up to U+007F".into(),
                            source: None,
                        });
                    }
                    bytes.push(c as u8);
            ...
            _ => {
                if let Some(enc) = self.get_encoding() {
                    ...
                    if had_errors {
                        return Err(Error::Format {
                            message: "Could not encode character".into(),
                            source: None,
                        });
                    }
                    Ok(res.into_owned())
                ...
                let u32s: Result<Vec<u32>, _> = input
                    ...
                    .map(|c| {
                        ...
                        if char::from_u32(val).is_some() {
                            ...
                            Err(format!("Invalid UTF-32BE: {val}"))
                        }
                    })
                    .collect();
                let u32s = u32s.map_err(|e| Error::Format {
                ...
                let u32s: Result<Vec<u32>, _> = input
                    ...
                    .map(|c| {
                        ...
                        if char::from_u32(val).is_some() {
                            ...
                            Err(format!("Invalid UTF-32LE: {val}"))
                        }
                    })
                    .collect();
                let u32s = u32s.map_err(|e| Error::Format {
