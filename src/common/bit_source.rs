...
impl<'a> BitSource<'a> {
    ...
    pub fn readBits(&mut self, numBits: usize) -> Result<u32> {
        if !(1..=32).contains(&numBits) || numBits > self.available() {
            return Err(Error::internal_with(format!(
                "readBits: numBits must be between 1 and 32, and less than available bits. numBits: {numBits}, available: {}",
                self.available()
            )));
        }
    ...
    pub fn peak_bits(&self, numBits: usize) -> Result<u32> {
        if !(1..=32).contains(&numBits) || numBits > self.available() {
            return Err(Error::internal_with(format!(
                "peak_bits: numBits must be between 1 and 32, and less than available bits. numBits: {numBits}, available: {}",
                self.available()
            )));
        }
