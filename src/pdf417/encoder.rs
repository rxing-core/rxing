...
;
 ...
 const PREFERRED_RATIO: f32 = 3.0f;

//1px in mm
 const DEFAULT_MODULE_WIDTH: f32 = 0.357f;

//mm
 const HEIGHT: f32 = 2.0f;
pub struct PDF417 {

     let barcode_matrix: BarcodeMatrix;
...
impl PDF417 {
    ...
    pub fn new( compact: bool) -> PDF417 {
        let .compact = compact;
    ...
    pub fn  get_barcode_matrix(&self) -> BarcodeMatrix  {
        return self.barcode_matrix;
    }
   ...
   */
    fn  calculate_number_of_rows( m: i32,  k: i32,  c: i32) -> i32  {
         let mut r: i32 = ((m + 1 + k) / c) + 1;
        if c * r >= (m + 1 + k + c) {
            r -= 1;
        }
        return r;
   ...
   */
    fn  get_number_of_pad_codewords( m: i32,  k: i32,  c: i32,  r: i32) -> i32  {
         let n: i32 = c * r - k;
        return  if n > m + 1 { n - m - 1 } else { 0 };
    }

    fn  encode_char( pattern: i32,  len: i32,  logic: &BarcodeRow)   {
         let mut map: i32 = 1 << len - 1;
        //Initialize to inverse of first bit
         let mut last: bool = (pattern & map) != 0;
         let mut width: i32 = 0;
         {
             let mut i: i32 = 0;
            while i < len {
                {
                     let black: bool = (pattern & map) != 0;
                    if last == black {
                        width += 1;
                    } else {
                        logic.add_bar(last, width);
                        last = black;
                        width = 1;
                    }
                    map >>= 1;
    ...
    fn  encode_low_level(&self,  full_codewords: &CharSequence,  c: i32,  r: i32,  error_correction_level: i32,  logic: &BarcodeMatrix)   {
         let mut idx: i32 = 0;
         {
             let mut y: i32 = 0;
            while y < r {
                {
                     let cluster: i32 = y % 3;
                    logic.start_row();
                    ::encode_char(START_PATTERN, 17, &logic.get_current_row());
                     let mut left: i32;
                     let mut right: i32;
                    if cluster == 0 {
                        left = (30 * (y / 3)) + ((r - 1) / 3);
                        ...
                        left = (30 * (y / 3)) + (error_correction_level * 3) + ((r - 1) % 3);
                        right = (30 * (y / 3)) + ((r - 1) / 3);
                    } else {
                        left = (30 * (y / 3)) + (c - 1);
                        right = (30 * (y / 3)) + (error_correction_level * 3) + ((r - 1) % 3);
                    }
                     let mut pattern: i32 = CODEWORD_TABLE[cluster][left];
                    ::encode_char(pattern, 17, &logic.get_current_row());
                     {
                         let mut x: i32 = 0;
                        while x < c {
                            {
                                pattern = CODEWORD_TABLE[cluster][full_codewords.char_at(idx)];
                                ::encode_char(pattern, 17, &logic.get_current_row());
                                idx += 1;
                            }
                            x += 1;
                    ...
                    if self.compact {
                        ...
                        ::encode_char(STOP_PATTERN, 1, &logic.get_current_row());
                    } else {
                        pattern = CODEWORD_TABLE[cluster][right];
                        ::encode_char(pattern, 17, &logic.get_current_row());
                        ::encode_char(STOP_PATTERN, 18, &logic.get_current_row());
                    }
                }
                y += 1;
             }
   ...
   */
    pub fn  generate_barcode_logic(&self,  msg: &String,  error_correction_level: i32)  -> /*  throws WriterException */Result<Void, Rc<Exception>>   {
        self.generate_barcode_logic(&msg, error_correction_level, false);
    }
   ...
   * @throws WriterException if the contents cannot be encoded in this format
   */
    pub fn  generate_barcode_logic(&self,  msg: &String,  error_correction_level: i32,  auto_e_c_i: bool)  -> /*  throws WriterException */Result<Void, Rc<Exception>>   {
        //1. step: High-level encoding
         let error_correction_code_words: i32 = PDF417ErrorCorrection::get_error_correction_codeword_count(error_correction_level);
         let high_level: String = PDF417HighLevelEncoder::encode_high_level(&msg, self.compaction, &self.encoding, auto_e_c_i);
         let source_code_words: i32 = high_level.length();
         let dimension: Vec<i32> = self.determine_dimensions(source_code_words, error_correction_code_words);
         let cols: i32 = dimension[0];
         let rows: i32 = dimension[1];
         let pad: i32 = ::get_number_of_pad_codewords(source_code_words, error_correction_code_words, cols, rows);
        //2. step: construct data codewords
        if source_code_words + error_correction_code_words + 1 > 929 {
            // +1 for symbol length CW
            throw WriterException::new(format!("Encoded message contains too many code words, message too big ({} bytes)", msg.length()));
        }
         let n: i32 = source_code_words + pad + 1;
         let sb: StringBuilder = StringBuilder::new(n);
        sb.append(n as char);
        sb.append(&high_level);
         {
             let mut i: i32 = 0;
            while i < pad {
                {
                    //PAD characters
                    sb.append(900 as char);
                }
                i += 1;
             }
         }

         let data_codewords: String = sb.to_string();
        //3. step: Error correction
         let ec: String = PDF417ErrorCorrection::generate_error_correction(&data_codewords, error_correction_level);
        //4. step: low-level encoding
        self.barcode_matrix = BarcodeMatrix::new(rows, cols);
        self.encode_low_level(format!("{}{}", data_codewords, ec), cols, rows, error_correction_level, self.barcode_matrix);
    }
   ...
   */
    fn  determine_dimensions(&self,  source_code_words: i32,  error_correction_code_words: i32) -> /*  throws WriterException */Result<Vec<i32>, Rc<Exception>>   {
         let mut ratio: f32 = 0.0f;
         let mut dimension: Vec<i32> = null;
         {
             let mut cols: i32 = self.min_cols;
            while cols <= self.max_cols {
                {
                     let rows: i32 = ::calculate_number_of_rows(source_code_words, error_correction_code_words, cols);
                    ...
                    if rows > self.max_rows {
                     ...
                     let new_ratio: f32 = ((17.0 * cols + 69.0) as f32 * DEFAULT_MODULE_WIDTH) / (rows * HEIGHT);
                    // ignore if previous ratio is closer to preferred ratio
                    if dimension != null && Math::abs(new_ratio - PREFERRED_RATIO) > Math::abs(ratio - PREFERRED_RATIO) {
                        continue;
                    }
                    ratio = new_ratio;
                    dimension =  : vec![i32; 2] = vec![cols, rows, ]
                    ;
                }
                cols += 1;
             }
        ...
        if dimension == null {
             let rows: i32 = ::calculate_number_of_rows(source_code_words, error_correction_code_words, self.min_cols);
            if rows < self.min_rows {
                dimension =  : vec![i32; 2] = vec![self.min_cols, self.min_rows, ]
                ;
            }
        }
        if dimension == null {
            throw WriterException::new("Unable to fit message in columns");
        }
        return Ok(dimension);
    }
   ...
   */
    pub fn  set_dimensions(&self,  max_cols: i32,  min_cols: i32,  max_rows: i32,  min_rows: i32)   {
        self.maxCols = max_cols;
        self.minCols = min_cols;
   ...
   */
    pub fn  set_compaction(&self,  compaction: &Compaction)   {
   ...
   */
    pub fn  set_encoding(&self,  encoding: &Charset)   {
        self.encoding = encoding;
    }
}

// NEW FILE: p_d_f417_error_correction.rs
/*
 * Copyright 2006 Jeremias Maerki in part, and ZXing Authors in part
 *
 * Licensed under the Apache License, Version 2.0 (the "License");
...
// NEW FILE: p_d_f417_high_level_encoder.rs
/*
 * Copyright 2006 Jeremias Maerki in part, and ZXing Authors in part
 *
 * Licensed under the Apache License, Version 2.0 (the "License");
...
// NEW FILE: p_d_f417_high_level_encoder_test_adapter.rs
/*
