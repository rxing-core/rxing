...
impl PDF417 {
     ...
     * @throws WriterException if the contents cannot be encoded in this format
     */
    pub fn generateBarcodeLogicWithAutoECI(
        &mut self,
        msg: &str,
        errorCorrectionLevel: u32,
        autoECI: bool,
        ...
        let highLevel = pdf_417_high_level_encoder::encodeHighLevel(
            msg,
            self.compaction,
            self.encoding,
            autoECI,
        ...
        let pad =
            Self::getNumberOfPadCodewords(sourceCodeWords, errorCorrectionCodeWords, cols, rows);
        ...
        let ec = pdf_417_error_correction::generateErrorCorrection(
            &dataCodewords,
    ...
    fn determineDimensions(
        ...
        for cols in self.minCols..=self.maxCols {
            ...
            let newRatio: f64 = ((17 * cols + 69) as f64 * DEFAULT_MODULE_WIDTH as f64)
                / (rows as f64 * HEIGHT as f64);

            // ignore if previous ratio is closer to preferred ratio
            if dimension.is_some()
                && (newRatio - PREFERRED_RATIO as f64).abs()
                    > (ratio - PREFERRED_RATIO as f64).abs()
            {
                continue;
            }
        ...
        if dimension.is_none() {
            let rows = Self::calculateNumberOfRows(
                sourceCodeWords,
