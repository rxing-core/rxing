...
use super::{BarcodeMatrix, Compaction, BarcodeRow, pdf_417_error_correction, pdf_417_high_level_encoder};
...
impl PDF417 {
  ...
  pub fn with_compact_hint( compact:bool) -> Self {
    Self {
        barcodeMatrix: None,
        compact,
  ...
  pub fn  getBarcodeMatrix(&self) -> &Option<BarcodeMatrix> {
    &self.barcodeMatrix
  }
   ...
   */
  fn calculateNumberOfRows( m:u32,  k:u32, c:u32) -> u32{
    let mut r = ((m + 1 + k) / c) + 1;
    if c * r >= (m + 1 + k + c) {
      r-=1;
    }
  ...
  fn encodeLowLevel(&self,  fullCodewords:&str,
                               c:u32,
                               r:u32,
    ...
    // for (int y = 0; y < r; y++) {
      ...
      if self.compact {
        Self::encodeChar(STOP_PATTERN, 1, logic.getCurrentRowMut()); // encodes stop line for compact pdf417
      } else {
        pattern = CODEWORD_TABLE[cluster][right as usize];
        Self::encodeChar(pattern, 17, logic.getCurrentRowMut());
   ...
   */
  pub fn generateBarcodeLogic(&mut self,  msg:&str,  errorCorrectionLevel:u32) -> Result<(),Exceptions> {
   ...
   * @throws WriterException if the contents cannot be encoded in this format
   */
  pub fn generateBarcodeLogicWithAutoECI(&mut self, msg:&str, errorCorrectionLevel:u32,  autoECI:bool) -> Result<(),Exceptions> {

    //1. step: High-level encoding
    let errorCorrectionCodeWords = pdf_417_error_correction::getErrorCorrectionCodewordCount(errorCorrectionLevel)?;
    let highLevel = pdf_417_high_level_encoder::encodeHighLevel(msg, self.compaction, self.encoding, autoECI)?;
    let sourceCodeWords = highLevel.chars().count() as u32;

    let dimension = self.determineDimensions(sourceCodeWords, errorCorrectionCodeWords)?;

    let cols = dimension[0];
    let rows = dimension[1];

    let pad = Self::getNumberOfPadCodewords(sourceCodeWords, errorCorrectionCodeWords, cols, rows);
    ...
    if sourceCodeWords + errorCorrectionCodeWords + 1 > 929 { // +1 for symbol length CW
      return Err(Exceptions::WriterException(
          format!("Encoded message contains too many code words, message too big ({} bytes)",msg.chars().count())));
    }
    ...
    let ec = pdf_417_error_correction::generateErrorCorrection(&dataCodewords, errorCorrectionLevel)?;

    //4. step: low-level encoding
     let mut barcode_matrix = BarcodeMatrix::new(rows as usize, cols as usize);
    self.encodeLowLevel(&format!("{}{}",dataCodewords, ec), cols, rows, errorCorrectionLevel, &mut barcode_matrix);
   ...
   */
  fn determineDimensions(&self, sourceCodeWords:u32,  errorCorrectionCodeWords:u32) -> Result<[u32;2],Exceptions> {
    let mut ratio = 0.0;
    let mut dimension = None;

    for cols in self.minCols..=self.maxCols {
    // for (int cols = minCols; cols <= maxCols; cols++) {

      let rows = Self::calculateNumberOfRows(sourceCodeWords, errorCorrectionCodeWords, cols);
      ...
      let newRatio: f32 = ( (17 * cols + 69) as f32 * DEFAULT_MODULE_WIDTH) / (rows as f32 * HEIGHT);

      // ignore if previous ratio is closer to preferred ratio
      if dimension.is_some() && (newRatio - PREFERRED_RATIO).abs() > (ratio - PREFERRED_RATIO).abs() {
        continue;
      }

      ratio = newRatio;
      dimension = Some([cols, rows]);
    }

    // Handle case when min values were larger than necessary
    if dimension.is_none() {
      let rows = Self::calculateNumberOfRows(sourceCodeWords, errorCorrectionCodeWords, self.minCols);
      if rows < self.minRows {
        dimension = Some([self.minCols, self.minRows]);
      }
    }
...
Ok(dim)
      ...
      Err(Exceptions::WriterException("Unable to fit message in columns".to_owned()))
    }
  }
   ...
   */
  pub fn setDimensions(&mut self,  maxCols:u32,  minCols:u32,  maxRows:u32,  minRows:u32) {
   ...
   */
  pub fn setCompaction(&mut self,  compaction:Compaction) {
...
/**
   ...
   */
  const CODEWORD_TABLE : [[u32;929];3]= [
