...
impl ASCIIEncoder {
    ...
    pub fn encode(&self, context: &EncoderContext) {
        //step B
        let n: i32 = HighLevelEncoder::determine_consecutive_digit_count(
            &context.get_message(),
            context.pos,
        );
        if n >= 2 {
            context.write_codeword(&::encode_a_s_c_i_i_digits(
                &context.get_message().char_at(context.pos),
                &context.get_message().char_at(context.pos + 1),
            ));
            context.pos += 2;
        } else {
            let c: char = context.get_current_char();
            let new_mode: i32 = HighLevelEncoder::look_ahead_test(
                &context.get_message(),
                context.pos,
                &self.get_encoding_mode(),
            );
...
impl Base256Encoder {
    ...
    pub fn encode(&self, context: &EncoderContext) {
        ...
        }
        {
            ...
            while i < c {
                {
                    context.write_codeword(&::randomize255_state(
                        &buffer.char_at(i),
                        context.get_codeword_count() + 1,
                    ));
                }
                i += 1;
            }
    ...
    fn encode_maximal(&self, context: &EncoderContext) {
        ...
        if backtrack_buffer_length != buffer.length() {
            ...
            if (rest == 2 && available != 2)
                || (rest == 1 && (last_char_size > 3 || available != 1))
            {
        ...
        while context.has_more_characters() {
            ...
            if !context.has_more_characters() {
                ...
                if (buffer.length() % 3) == 2 && available != 2 {
                    last_char_size =
                        self.backtrack_one_character(context, &buffer, &removed, last_char_size);
                }
...
impl DataMatrixSymbolInfo144 {
    fn new() -> Self {
        Self {
            super_type: SymbolInfo::new(false, 1558, 620, 22, 22, 36, -1, 62),
        }
    }
...
impl DefaultPlacement {
    ...
    pub fn place(&self) {
        ...
        loop {
            ...
            if !((row < self.numrows) || (col < self.numcols)) {
                break;
            }
        }
    ...
    fn handle_e_o_d(context: &EncoderContext, buffer: &CharSequence) {
        ...
        if count == 1 {
            ...
            let mut available: i32 =
                context.get_symbol_info().get_data_capacity() - context.get_codeword_count();
...
impl EncoderContext {
    ...
    pub fn update_symbol_info(&self, len: i32) {
        if self.symbolInfo == null || len > self.symbolInfo.get_data_capacity() {
            self.symbolInfo =
                SymbolInfo::lookup(len, self.shape, self.min_size, self.max_size, true);
        }
    }
 ...
 * See FACTORS.
 */
const FACTOR_SETS: vec![Vec<i32>; 16] =
    vec![5, 7, 10, 11, 12, 14, 18, 20, 24, 28, 36, 42, 48, 56, 62, 68];

/**
 * Precomputed polynomial factors for ECC 200.
 */
const FACTORS: vec![vec![Vec<Vec<i32>>; 68]; 16] = vec![
                ...
                while block < block_count {
                    {
                        ...
                        let ecc: String =
                            ::create_e_c_c_block(&temp.to_string(), error_sizes[block]);
                        let mut pos: i32 = 0;
                        {
                            let mut e: i32 = block;
                            while e < error_sizes[block] * block_count {
                                {
                                    sb.set_char_at(
                                        symbol_info.get_data_capacity() + e,
                                        &ecc.char_at(pos += 1),
                                    );
                                }
                                e += block_count;
                        ...
                        while k > 0 {
                            {
                                if m != 0 && poly[k] != 0 {
                                    ecc[k] =
                                        (ecc[k - 1] ^ ALOG[(LOG[m] + LOG[poly[k]]) % 255]) as char;
                                } else {
                                    ecc[k] = ecc[k - 1];
                                }
                            }
...
impl HighLevelEncoder {
     ...
     * @return the encoded message (the char values range from 0 to 255)
     */
    pub fn encode_high_level_complex(
        msg: &String,
        shape: &SymbolShapeHint,
        min_size: &Dimension,
        max_size: &Dimension,
     ...
     * @return the encoded message (the char values range from 0 to 255)
     */
    pub fn encode_high_level_comples_encforce(
        msg: &String,
        shape: &SymbolShapeHint,
        min_size: &Dimension,
        max_size: &Dimension,
        ...
        let encoders: vec![Vec<Encoder>; 6] = vec![
            ASCIIEncoder::new(),
            c40_encoder,
            TextEncoder::new(),
            X12Encoder::new(),
        ...
        if len < capacity
            && encoding_mode != ASCII_ENCODATION
            && encoding_mode != BASE256_ENCODATION
            && encoding_mode != EDIFACT_ENCODATION
        {
            //Unlatch (254)
    ...
    fn look_ahead_test_intern(msg: &CharSequence, startpos: i32, current_mode: i32) -> i32 {
        ...
        while true {
            ...
            if chars_processed >= 4 {
                ...
                Arrays::fill(&int_char_counts, 0);
                ::find_minimums(&char_counts, &int_char_counts, Integer::MAX_VALUE, &mins);
                if int_char_counts[ASCII_ENCODATION]
                    < ::min(
                        int_char_counts[BASE256_ENCODATION],
                        int_char_counts[C40_ENCODATION],
                        int_char_counts[TEXT_ENCODATION],
                        int_char_counts[X12_ENCODATION],
                        int_char_counts[EDIFACT_ENCODATION],
                    )
                {
                    return ASCII_ENCODATION;
                }
                if int_char_counts[BASE256_ENCODATION] < int_char_counts[ASCII_ENCODATION]
                    || int_char_counts[BASE256_ENCODATION] + 1
                        < ::min(
                            int_char_counts[C40_ENCODATION],
                            int_char_counts[TEXT_ENCODATION],
                            int_char_counts[X12_ENCODATION],
                ...
                if int_char_counts[EDIFACT_ENCODATION] + 1
                    < ::min(
                        int_char_counts[BASE256_ENCODATION],
                        int_char_counts[C40_ENCODATION],
                        int_char_counts[TEXT_ENCODATION],
                        int_char_counts[X12_ENCODATION],
                        int_char_counts[ASCII_ENCODATION],
                    )
                {
                    return EDIFACT_ENCODATION;
                }
                if int_char_counts[TEXT_ENCODATION] + 1
                    < ::min(
                        int_char_counts[BASE256_ENCODATION],
                        int_char_counts[C40_ENCODATION],
                        int_char_counts[EDIFACT_ENCODATION],
                ...
                if int_char_counts[X12_ENCODATION] + 1
                    < ::min(
                        int_char_counts[BASE256_ENCODATION],
                        int_char_counts[C40_ENCODATION],
                        int_char_counts[EDIFACT_ENCODATION],
                ...
                if int_char_counts[C40_ENCODATION] + 1
                    < ::min(
                        int_char_counts[ASCII_ENCODATION],
                        int_char_counts[BASE256_ENCODATION],
                        int_char_counts[EDIFACT_ENCODATION],
...
const C40_SHIFT2_CHARS: vec![Vec<char>; 27] = vec![
    '!', '"', '#', '$', '%', '&', '\'', '(', ')', '*', '+', ',', '-', '.', '/', ':', ';', '<', '=',
    '>', '?', '@', '[', '\\', ']', '^', '_',
];
...
impl MinimalEncoder {
    ...
    pub fn encode_high_level(
        msg: &String,
        priority_charset: &Charset,
        fnc1: i32,
        shape: &SymbolShapeHint,
    ) -> String {
        let macro_id: i32 = 0;
        if msg.starts_with(HighLevelEncoder::MACRO_05_HEADER)
            && msg.ends_with(HighLevelEncoder::MACRO_TRAILER)
        {
            macro_id = 5;
            msg = msg.substring(
        ...
        } else if msg.starts_with(HighLevelEncoder::MACRO_06_HEADER)
            && msg.ends_with(HighLevelEncoder::MACRO_TRAILER)
        {
            macro_id = 6;
            msg = msg.substring(
    ...
    fn encode(
        ...
        return ::encode_minimally(Input::new(&input, &priority_charset, fnc1, shape, macro_id))
            .get_bytes();
    }

    fn add_edge(edges: &Vec<Vec<Edge>>, edge: &Edge) {
        let vertex_index: i32 = edge.fromPosition + edge.characterLength;
        if edges[vertex_index][edge.get_end_mode().ordinal()] == null
            || edges[vertex_index][edge.get_end_mode().ordinal()].cachedTotalSize
    ...
    fn get_number_of_c40_words(
        input: &Input,
        from: i32,
        c40: bool,
        character_length: &Vec<i32>,
    ) -> i32 {
            ...
            while i < input.length() {
                {
                    ...
                    if c40 && HighLevelEncoder::is_native_c40(ci)
                        || !c40 && HighLevelEncoder::is_native_text(ci)
                    {
                        thirds_count += 1;
                        ...
                        if ascii_value >= 128
                            && (c40 && HighLevelEncoder::is_native_c40((ascii_value - 128) as char)
                                || !c40
                                    && HighLevelEncoder::is_native_text(
                                        (ascii_value - 128) as char,
                                    ))
                        {
                            thirds_count += 3;
                        } else {
                    ...
                    if thirds_count % 3 == 0
                        || ((thirds_count - 2) % 3 == 0 && i + 1 == input.length())
                    {
    ...
    fn add_edges(input: &Input, edges: &Vec<Vec<Edge>>, from: i32, previous: &Edge) {
        ...
        if previous == null || previous.get_end_mode() != Mode::EDF {
            if HighLevelEncoder::is_digit(ch)
                && input.have_n_characters(from, 2)
                && HighLevelEncoder::is_digit(&input.char_at(from + 1))
            {
            ...
            for mode in modes {
                ...
                if ::get_number_of_c40_words(input, from, mode == Mode::C40, &character_length) > 0
                {
                    ::add_edge(
                        edges,
                        Edge::new(input, mode, from, character_length[0], previous),
                    );
                }
            }
            if input.have_n_characters(from, 3)
                && HighLevelEncoder::is_native_x12(&input.char_at(from))
                && HighLevelEncoder::is_native_x12(&input.char_at(from + 1))
                && HighLevelEncoder::is_native_x12(&input.char_at(from + 2))
            {
        ...
        let mut i: i32;
        {
            ...
            while i < 3 {
                {
                    let pos: i32 = from + i;
                    if input.have_n_characters(pos, 1)
                        && HighLevelEncoder::is_native_e_d_i_f_a_c_t(&input.char_at(pos))
                    {
        ...
        if i == 3
            && input.have_n_characters(from, 4)
            && HighLevelEncoder::is_native_e_d_i_f_a_c_t(&input.char_at(from + 3))
        {
    ...
    const all_codeword_capacities: vec![Vec<i32>; 28] = vec![
        3, 5, 8, 10, 12, 16, 18, 22, 30, 32, 36, 44, 49, 62, 86, 114, 144, 174, 204, 280, 368, 456,
        576, 696, 816, 1050, 1304, 1558,
    ];

    const square_codeword_capacities: vec![Vec<i32>; 24] = vec![
...
impl Edge {
    fn new(
        ...
        let mut size: i32 = if previous != null {
        ...
        */
        match mode {
            ASCII => {
                size += 1;
                if input.is_e_c_i(from_position)
                    || ::is_extended_a_s_c_i_i(
                        &input.char_at(from_position),
                        &input.get_f_n_c1_character(),
                    )
                {
                    size += 1;
                }
                if previous_mode == Mode::C40
                    || previous_mode == Mode::TEXT
                    || previous_mode == Mode::X12
                {
            ...
            B256 => {
                ...
                if previous_mode == Mode::ASCII {
                    //latch to B256
                    size += 1;
                } else if previous_mode == Mode::C40
                    || previous_mode == Mode::TEXT
                    || previous_mode == Mode::X12
                {
    ...
    * */
    fn get_end_mode(&self) -> Mode {
        if self.mode == Mode::EDF {
            ...
            if last_a_s_c_i_i > 0
                && self.get_codewords_remaining(self.cached_total_size + last_a_s_c_i_i)
                    <= 2 - last_a_s_c_i_i
            {
                return Mode::ASCII;
            }
        ...
        if self.mode == Mode::C40 || self.mode == Mode::TEXT || self.mode == Mode::X12 {
            ...
            if self.from_position + self.character_length >= self.input.length()
                && self.get_codewords_remaining(self.cached_total_size) == 0
            {
                return Mode::ASCII;
            }
            let last_a_s_c_i_i: i32 = self.get_last_a_s_c_i_i();
    ...
    **/
    fn get_last_a_s_c_i_i(&self) -> i32 {
        ...
        if length - from == 1 {
            if ::is_extended_a_s_c_i_i(
                &self.input.char_at(from),
                &self.input.get_f_n_c1_character(),
            ) {
                return 0;
            }
            return 1;
        ...
        if length - from == 2 {
            if ::is_extended_a_s_c_i_i(
                ...
                &self.input.char_at(from + 1),
                &self.input.get_f_n_c1_character(),
            ) {
                return 0;
            }
            if HighLevelEncoder::is_digit(&self.input.char_at(from))
                && HighLevelEncoder::is_digit(&self.input.char_at(from + 1))
            {
                return 1;
            }
            return 2;
        ...
        if length - from == 3 {
            if HighLevelEncoder::is_digit(&self.input.char_at(from))
                && HighLevelEncoder::is_digit(&self.input.char_at(from + 1))
                && !::is_extended_a_s_c_i_i(
                    &self.input.char_at(from + 2),
                    &self.input.get_f_n_c1_character(),
                )
            {
                return 2;
            }
            if HighLevelEncoder::is_digit(&self.input.char_at(from + 1))
                && HighLevelEncoder::is_digit(&self.input.char_at(from + 2))
                && !::is_extended_a_s_c_i_i(
                    &self.input.char_at(from),
                    &self.input.get_f_n_c1_character(),
                )
        ...
        if HighLevelEncoder::is_digit(&self.input.char_at(from))
            && HighLevelEncoder::is_digit(&self.input.char_at(from + 1))
            && HighLevelEncoder::is_digit(&self.input.char_at(from + 2))
            && HighLevelEncoder::is_digit(&self.input.char_at(from + 3))
        {
            return 2;
        }
        return 0;
    ...
    **/
            ...
            while i < result.len() {
                {
                    ::set_c40_word(
                        &result,
                        i,
                        &::get_x12_value(&self.input.char_at(self.from_position + i / 2 * 3)),
                        &::get_x12_value(&self.input.char_at(self.from_position + i / 2 * 3 + 1)),
                        &::get_x12_value(&self.input.char_at(self.from_position + i / 2 * 3 + 2)),
                    );
                }
                i += 2;
            }
    ...
    fn get_shift_value(c: char, c40: bool, fnc1: i32) -> i32 {
        return if (c40 && ::is_in_c40_shift1_set(c) || !c40 && ::is_in_text_shift1_set(c)) {
            0
        } else {
            if (c40 && ::is_in_c40_shift2_set(c, fnc1) || !c40 && ::is_in_text_shift2_set(c, fnc1))
            {
                1
            } else {
                2
    ...
    fn get_c40_words(&self, c40: bool, fnc1: i32) -> Vec<i8> {
            ...
            while i < self.character_length {
                {
                        ...
                        || !c40 && HighLevelEncoder::is_native_text(ci)
                    {
                        ...
                        if c40 && HighLevelEncoder::is_native_c40(ascii_value)
                            || !c40 && HighLevelEncoder::is_native_text(ascii_value)
                        {
                            //Shift 2
                            ...
                            c40_values.add(shift_value as i8);
                            c40_values
                                .add(::get_c40_value(c40, shift_value, ascii_value, fnc1) as i8);
                        }
                    }
                }
                i += 1;
        ...
        if (c40_values.size() % 3) != 0 {
            assert!(
                (c40_values.size() - 2) % 3 == 0
                    && self.from_position + self.character_length == self.input.length()
            );
            // pad with 0 (Shift 1)
            ...
            while i < c40_values.size() {
                {
                    ::set_c40_word(
                        &result,
                        byte_index,
                        c40_values.get(i) & 0xff,
                        c40_values.get(i + 1) & 0xff,
                        c40_values.get(i + 2) & 0xff,
                    );
                    byte_index += 2;
                }
                i += 3;
    ...
    fn get_e_d_f_bytes(&self) -> Vec<i8> {
        ...
        let end_pos: i32 = Math::min(
            self.from_position + self.character_length - 1,
            self.input.length() - 1,
        );
        {
            let mut i: i32 = 0;
    ...
    fn get_data_bytes(&self) -> Vec<i8> {
        match self.mode {
            ASCII => {
                ...
                } else if ::is_extended_a_s_c_i_i(
                    &self.input.char_at(self.from_position),
                    &self.input.get_f_n_c1_character(),
                ) {
                    return ::get_bytes(235, self.input.char_at(self.from_position) - 127);
                } else if self.character_length == 2 {
                    return ::get_bytes(
                        (self.input.char_at(self.from_position) - '0') * 10
                            + self.input.char_at(self.from_position + 1)
                            - '0'
                            + 130,
                    );
...
impl Result {
    fn new(solution: &Edge) -> Result {
        ...
        if (solution.mode == Mode::C40 || solution.mode == Mode::TEXT || solution.mode == Mode::X12)
            && solution.get_end_mode() != Mode::ASCII
        {
        ...
        }
        {
            ...
            while i < randomize_postfix_length.size() {
                {
                    ::apply_random_pattern(
                        &bytes_a_l,
                        bytes_a_l.size() - randomize_postfix_length.get(i),
...
const PROD_SYMBOLS: vec![Vec<SymbolInfo>; 30] = vec![
    SymbolInfo::new_simple(false, 3, 5, 8, 8, 1),
    SymbolInfo::new_simple(false, 5, 7, 10, 10, 1), /*rect*/
    SymbolInfo::new_simple(true, 5, 7, 16, 6, 1),
    SymbolInfo::new_simple(false, 8, 10, 12, 12, 1), /*rect*/
    SymbolInfo::new_simple(true, 10, 11, 14, 6, 2),
    SymbolInfo::new_simple(false, 12, 12, 14, 14, 1), /*rect*/
    SymbolInfo::new_simple(true, 16, 14, 24, 10, 1),
    SymbolInfo::new_simple(false, 18, 14, 16, 16, 1),
    SymbolInfo::new_simple(false, 22, 18, 18, 18, 1), /*rect*/
    SymbolInfo::new_simple(true, 22, 18, 16, 10, 2),
    SymbolInfo::new_simple(false, 30, 20, 20, 20, 1), /*rect*/
    SymbolInfo::new_simple(true, 32, 24, 16, 14, 2),
    SymbolInfo::new_simple(false, 36, 24, 22, 22, 1),
    SymbolInfo::new_simple(false, 44, 28, 24, 24, 1), /*rect*/
    SymbolInfo::new_simple(true, 49, 28, 22, 14, 2),
    SymbolInfo::new_simple(false, 62, 36, 14, 14, 4),
    SymbolInfo::new_simple(false, 86, 42, 16, 16, 4),
    SymbolInfo::new_simple(false, 114, 48, 18, 18, 4),
    SymbolInfo::new_simple(false, 144, 56, 20, 20, 4),
    SymbolInfo::new_simple(false, 174, 68, 22, 22, 4),
    SymbolInfo::new(false, 204, 84, 24, 24, 4, 102, 42),
    SymbolInfo::new(false, 280, 112, 14, 14, 16, 140, 56),
    SymbolInfo::new(false, 368, 144, 16, 16, 16, 92, 36),
    SymbolInfo::new(false, 456, 192, 18, 18, 16, 114, 48),
    SymbolInfo::new(false, 576, 224, 20, 20, 16, 144, 56),
    SymbolInfo::new(false, 696, 272, 22, 22, 16, 174, 68),
    SymbolInfo::new(false, 816, 336, 24, 24, 16, 136, 56),
    SymbolInfo::new(false, 1050, 408, 18, 18, 36, 175, 68),
    SymbolInfo::new(false, 1304, 496, 20, 20, 36, 163, 62),
    DataMatrixSymbolInfo144::new(),
];
...
impl SymbolInfo {
    ...
    pub fn lookup_allow_fail(
        data_codewords: i32,
        allow_rectangular: bool,
        fail: bool,
    ) -> SymbolInfo {
        let shape: SymbolShapeHint = if allow_rectangular {
    ...
    pub fn lookup_complex(
        ...
        for symbol in symbols {
            ...
            if min_size != null
                && (symbol.get_symbol_width() < min_size.get_width()
                    || symbol.get_symbol_height() < min_size.get_height())
            {
                continue;
            }
            if max_size != null
                && (symbol.get_symbol_width() > max_size.get_width()
                    || symbol.get_symbol_height() > max_size.get_height())
            {
                continue;
            }
    ...
    pub fn to_string(&self) -> String {
        return format!(
            ...
            self.matrix_width,
            self.matrix_height,
            self.get_symbol_width(),
            self.get_symbol_height(),
            self.get_symbol_data_width(),
            self.get_symbol_data_height(),
            self.data_capacity,
...
impl X12Encoder {
    ...
    fn handle_e_o_d(&self, context: &EncoderContext, buffer: &StringBuilder) {
        ...
        let count: i32 = buffer.length();
        context.pos -= count;
        if context.get_remaining_characters() > 1
            || available > 1
            || context.get_remaining_characters() != available
        {
