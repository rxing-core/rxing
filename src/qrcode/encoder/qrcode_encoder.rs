...
pub fn encode_with_hints(
    ...
    if has_encoding_hint {
        if let Some(v) = &hints.CharacterSet {
            encoding = Some(
                CharacterSet::get_character_set_by_name(v)
                    .ok_or(Error::invalid_input_with("character set hint", v.clone()))?,
            )
        }
    }
...
fn chooseVersion(numInputBits: u32, ecLevel: &ErrorCorrectionLevel) -> Result<VersionRef> {
    ...
    Err(Error::invalid_input_with(
        "data size",
        format!("data too big {numInputBits}/{ecLevel:?}"),
    ))
}
...
pub fn terminateBits(num_data_bytes: u32, bits: &mut BitArray) -> Result<()> {
    ...
    if bits.get_size() > capacity as usize {
        return Err(Error::invalid_input_with(
            "data size",
            format!("data bits {num_data_bytes} cannot fit in the QR Code {capacity} > "),
        ));
    }
    ...
    if bits.get_size() != capacity as usize {
        return Err(Error::internal_with("bits size does not equal capacity"));
    }
    Ok(())
}
...
pub fn getNumDataBytesAndNumECBytesForBlockID(
    ...
    if block_id >= num_rsblocks {
        return Err(Error::internal_with("block ID too large"));
    }
    ...
    if num_ec_bytes_in_group1 != numEcBytesInGroup2 {
        return Err(Error::internal_with("EC bytes mismatch"));
    }
    // 5 = 4 + 1.
    if num_rsblocks != num_rs_blocks_in_group1 + num_rs_blocks_in_group2 {
        return Err(Error::internal_with("RS blocks mismatch"));
    }
    // 196 = (13 + 26) * 4 + (14 + 26) * 1
    if num_total_bytes
    ...
    {
        return Err(Error::internal_with("total bytes mismatch"));
    }
...
pub fn interleaveWithECBytes(
    ...
    if num_data_bytes != data_bytes_offset as u32 {
        return Err(Error::internal_with("data bytes does not match offset"));
    }
...
pub fn appendBytes(
    ...
    match mode {
        ...
        _ => Err(Error::internal_with(format!("Invalid mode: {mode:?}"))),
    }
}
...
pub fn appendAlphanumericBytes(content: &str, bits: &mut BitArray) -> Result<()> {
    ...
    while i < length {
        ...
        if code1 == -1 {
            return Err(Error::internal_with("invalid alphanumeric code"));
        }
        if i + 1 < length {
...
pub fn appendKanjiBytes(content: &str, bits: &mut BitArray) -> Result<()> {
    ...
    if bytes.len() % 2 != 0 {
        return Err(Error::internal_with("Kanji byte size not even"));
    }
    ...
    while i < max_i {
        ...
        if subtracted == -1 {
            return Err(Error::internal_with("Invalid byte sequence"));
        }
