...
pub fn calculateBCHCode(value: u32, poly: u32) -> Result<u32> {
    if poly == 0 {
        return Err(Error::internal_with("calculateBCHCode: 0 polynomial"));
    }
    let mut value = value;
...
pub fn makeTypeInfoBits(
    ...
    if !QRCode::isValidMaskPattern(maskPattern as i32) {
        return Err(Error::internal_with("invalid mask pattern"));
    }
...
pub fn embedDarkDotAtLeftBottomCorner(matrix: &mut ByteMatrix) -> Result<()> {
    if matrix.get(8, matrix.getHeight() - 8) == 0 {
        return Err(Error::internal_with(
            "matrix lonely dark dot JISX0510:2004 (p.46) already zero",
        ));
    }
